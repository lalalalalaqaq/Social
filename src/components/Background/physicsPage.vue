<template>
  <Renderer ref="renderer" pointer resize shadow @click="randomColors">
    ...
    <Scene background="white">
      ...
      <SpotLight color="#ffffff" :intensity="0.5" :position="{ x: 0, y: 3, z: 2 }" :angle="Math.PI / 8" :penumbra="0.1"
        cast-shadow :shadow-map-size="{ width: 2048, height: 2048 }" />
      <SpotLight color="#ff0000" :intensity="0.5" :position="{ x: 0, y: -3, z: 2 }" :angle="Math.PI / 8" :penumbra="0.1"
        cast-shadow :shadow-map-size="{ width: 2048, height: 2048 }" />
      <CannonWorld :gravity="{ x: 0, y: 0, z: 0 }" @before-step="onBeforeStep">
        ...
        <Sphere v-for="i in 3" :key="`sphere-${i}`" :radius="0.25" :position="{ x: i - 2 }" :width-segments="16"
          :height-segments="16" receive-shadow>
          <PhongMaterial />
        </Sphere>
      </CannonWorld>
    </Scene>
